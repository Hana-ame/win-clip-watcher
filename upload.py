...
def upload_file(
    ...
    # 3. 发送 PUT 请求
    headers = {"Content-Type": mime_type, "X-File-Name": urllib.parse.quote(filename)}
    ...
    try:
        ...
        public_url = (
            f"https://upload.moonchan.xyz/api/{file_id}/{urllib.parse.quote(filename)}"
        )
        return public_url
...
def gzip_compress_text(text: str, filename: str = "compressed.txt.gz") -> bytes:
    ...
    with gzip.GzipFile(
        fileobj=compressed_buffer, mode="wb", filename=filename, mtime=0
    ) as gz:
        gz.write(text_bytes)
...
def main(original_text: str):
    ...
    uploaded_url = upload_file(data_bytes=compressed_data, custom_name="example.txt.gz")
